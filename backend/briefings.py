...
BRIEFINGS: tuple[Briefing, ...] = (
    ...
    Briefing(
        ...
        before_mission_index=30,
        missions_range="31-35",
        steps=(
            BriefingStep(
                type="lesson",
                title="Keys and values",
                text="A dictionary stores values under named keys. A key describes what its value means.",
                retry_text='In {"name": "Mompy"}, "name" is the key and "Mompy" is its value.',
            ),
            BriefingStep(
                type="lesson",
                title="Reading a key",
                text="Use brackets and a key to read one value, such as profile[\"name\"].",
                retry_text="The key goes inside brackets after the dictionary variable.",
            ),
            BriefingStep(
                type="lesson",
                title="Adding and updating",
                text="Assigning a value to a new key adds it. Assigning to an existing key updates it.",
                retry_text='profile["level"] = 2 stores 2 under the level key.',
            ),
            BriefingStep(
                type="lesson",
                title="Safe lookup",
                text="get() can return a fallback instead of stopping when a key is missing.",
                retry_text='profile.get("mode", "offline") returns offline when mode is absent.',
            ),
            check(
                "A dictionary key is used to:",
                "identify and access a stored value",
                "repeat a loop forever",
                "indent a function",
                "Correct. Keys identify dictionary values.",
                "Not yet. A key names a value stored in a dictionary.",
            ),
            check(
                "get() is useful when:",
                "a key may be missing",
                "a loop must count backward",
                "a function has no parameters",
                "Correct. get() can safely use a fallback.",
                "Almost. get() reads a key and can provide a fallback.",
            ),
        ),
    ),
    Briefing(
        ...
        before_mission_index=35,
        missions_range="36-40",
        steps=(
            BriefingStep(
                type="lesson",
                title="Condition-controlled repetition",
                text="A while loop repeats while its condition is true. Python checks the condition before every repetition.",
                retry_text="Read while as: keep doing this while the condition remains true.",
            ),
            BriefingStep(
                type="lesson",
                title="Update the condition",
                text="The loop usually changes a counter or another value so the condition can eventually become false.",
                retry_text="If count never changes, count <= 3 may stay true forever. Update count inside the loop.",
            ),
            BriefingStep(
                type="lesson",
                title="Counters and accumulators",
                text="A counter tracks repetitions. An accumulator keeps a result, such as a running total.",
                retry_text="count can advance the loop while total collects the values.",
            ),
            BriefingStep(
                type="lesson",
                title="Lists with while",
                text="An index can move from 0 up to the list length, visiting one item per repetition.",
                retry_text="Use index < len(items), read items[index], then increase index.",
            ),
            check(
                "A while loop continues when:",
                "its condition is true",
                "its list is always empty",
                "print is removed",
                "Correct. while repeats while its condition is true.",
                "Not yet. Python checks the while condition before each repetition.",
            ),
            check(
                "Why update a counter inside while?",
                "to make progress toward ending the loop",
                "to turn every value into text",
                "to create a dictionary key",
                "Correct. The update lets the condition eventually become false.",
                "Almost. Without an update, a true condition may never change.",
            ),
        ),
    ),
)
